...
_path_list = [
    os.getenv("LIBUNICORN_PATH", None),
...
if _uc is None:
    for _path in _path_list:
        ...
        if _uc is not None:
            ...
            print(
                "Found an old style dynamic library libunicorn.so, consider checking your installation",
                file=sys.stderr,
            )
            break
...
_setup_prototype(
    _uc, "uc_open", ucerr, ctypes.c_uint, ctypes.c_uint, ctypes.POINTER(uc_engine)
)
...
_setup_prototype(
    _uc, "uc_mem_unmap", ucerr, uc_engine, ctypes.c_uint64, ctypes.c_size_t
)
_setup_prototype(
    _uc,
    "uc_mem_protect",
...
_setup_prototype(
    _uc, "uc_query", ucerr, uc_engine, ctypes.c_uint32, ctypes.POINTER(ctypes.c_size_t)
)
...
_setup_prototype(
    _uc, "uc_context_reg_read", ucerr, uc_context, ctypes.c_int, ctypes.c_void_p
)
_setup_prototype(
    _uc, "uc_context_reg_write", ucerr, uc_context, ctypes.c_int, ctypes.c_void_p
)
...
UC_HOOK_INTR_CB = ctypes.CFUNCTYPE(None, uc_engine, ctypes.c_uint32, ctypes.c_void_p)
...
UC_HOOK_INSN_OUT_CB = ctypes.CFUNCTYPE(
    None, uc_engine, ctypes.c_uint32, ctypes.c_int, ctypes.c_uint32, ctypes.c_void_p
)
UC_HOOK_INSN_SYSCALL_CB = ctypes.CFUNCTYPE(None, uc_engine, ctypes.c_void_p)
UC_HOOK_INSN_SYS_CB = ctypes.CFUNCTYPE(
    ctypes.c_uint32, uc_engine, ctypes.c_uint32, ctypes.c_void_p, ctypes.c_void_p
)
...
def reg_read(reg_read_func, arch, reg_id, opt=None):
    if arch == uc.UC_ARCH_X86:
        if reg_id in [
            x86_const.UC_X86_REG_IDTR,
            x86_const.UC_X86_REG_GDTR,
            x86_const.UC_X86_REG_LDTR,
    ...
    if arch == uc.UC_ARCH_ARM64:
        ...
        elif reg_id in range(
            arm64_const.UC_ARM64_REG_Q0, arm64_const.UC_ARM64_REG_Q31 + 1
        ) or reg_id in range(
...
class Uc(object):
    ...
    def reg_read(
        self, reg_id: int, opt: Union[None, int, ARMCPReg, ARM64CPReg] = None
    ) -> Union[int, X86MMRReg, X86FPReg]:
        return reg_read(
            functools.partial(_uc.uc_reg_read, self._uch), self._arch, reg_id, opt
        )
    ...
    def reg_write(
        self,
        reg_id: int,
        value: Union[int, ARMCPRegValue, ARM64CPRegValue, X86MMRReg, X86FPReg],
    ):
        return reg_write(
            functools.partial(_uc.uc_reg_write, self._uch), self._arch, reg_id, value
        )
    ...
    def mmio_map(
        ...
        address: int,
        size: int,
        read_cb: UC_MMIO_READ_TYPE,
        user_data_read: Any,
        write_cb: UC_MMIO_WRITE_TYPE,
        user_data_write: Any,
    ):
        internal_read_cb = ctypes.cast(
            UC_MMIO_READ_CB(self._mmio_map_read_cb), UC_MMIO_READ_CB
        )
        internal_write_cb = ctypes.cast(
            UC_MMIO_WRITE_CB(self._mmio_map_write_cb), UC_MMIO_WRITE_CB
        )
    ...
    def _hook_insn_sys_cb(self, handle, reg, pcp_reg, user_data):
        ...
        uc_arm64_cp_reg_tuple = namedtuple(
            "uc_arm64_cp_reg_tuple", ["crn", "crm", "op0", "op1", "op2", "val"]
        )
        ...
        return cb(
            self,
            reg,
            uc_arm64_cp_reg_tuple(
                cp_reg.crn, cp_reg.crm, cp_reg.op0, cp_reg.op1, cp_reg.op2, cp_reg.val
            ),
            data,
        )
    ...
    def ctl_get_exits(self):
        ...
        self.ctl(
            self.__ctl_r(uc.UC_CTL_UC_EXITS, 2),
    ...
    def ctl_set_exits(self, exits: List[int]):
        ...
        for idx, exit in enumerate(exits):
            arr[idx] = exit
        self.ctl(
            self.__ctl_w(uc.UC_CTL_UC_EXITS, 2),
            ctypes.cast(arr, ctypes.c_void_p),
            ctypes.c_size_t(len(exits)),
        )
    ...
    def ctl_remove_cache(self, addr: int, end: int):
        self.__ctl_w_2_arg(
            uc.UC_CTL_TB_REMOVE_CACHE, addr, end, ctypes.c_uint64, ctypes.c_uint64
        )

    def ctl_request_cache(self, addr: int):
        return self.__ctl_rw_1_1_arg(
            uc.UC_CTL_TB_REQUEST_CACHE, addr, ctypes.c_uint64, uc_tb
        )
    ...
    def hook_add(
        self,
        htype: int,
        callback: UC_HOOK_CALLBACK_TYPE,
        user_data: Any = None,
        begin: int = 1,
        end: int = 0,
        arg1: int = 0,
        ...
        if htype == uc.UC_HOOK_INSN:
            ...
            if arg1 == x86_const.UC_X86_INS_IN:  # IN instruction
                cb = ctypes.cast(
                    UC_HOOK_INSN_IN_CB(self._hook_insn_in_cb), UC_HOOK_INSN_IN_CB
                )
            if arg1 == x86_const.UC_X86_INS_OUT:  # OUT instruction
                cb = ctypes.cast(
                    UC_HOOK_INSN_OUT_CB(self._hook_insn_out_cb), UC_HOOK_INSN_OUT_CB
                )
            if arg1 in (
            ...
            if arg1 == x86_const.UC_X86_INS_CPUID:  # CPUID instruction
                cb = ctypes.cast(
                    UC_HOOK_INSN_CPUID_CB(self._hook_insn_cpuid_cb),
                    UC_HOOK_INSN_CPUID_CB,
                )
            if arg1 in (
                arm64_const.UC_ARM64_INS_MRS,
                arm64_const.UC_ARM64_INS_MSR,
                arm64_const.UC_ARM64_INS_SYS,
                arm64_const.UC_ARM64_INS_SYSL,
            ):
                cb = ctypes.cast(
                    UC_HOOK_INSN_SYS_CB(self._hook_insn_sys_cb), UC_HOOK_INSN_SYS_CB
                )
            ...
            status = _uc.uc_hook_add(
                ...
                htype,
                ctypes.cast(
                    UC_HOOK_TCG_OPCODE_CB(self._hook_tcg_op_cb), UC_HOOK_TCG_OPCODE_CB
                ),
                ctypes.cast(self._callback_count, ctypes.c_void_p),
                ctypes.c_uint64(begin),
                ctypes.c_uint64(end),
            ...
            cb = ctypes.cast(
                UC_HOOK_EDGE_GEN_CB(self._hook_edge_gen_cb), UC_HOOK_EDGE_GEN_CB
            )
            status = _uc.uc_hook_add(
                self._uch,
                ctypes.byref(_h2),
    ...
    def mem_regions(self):
        ...
        status = _uc.uc_mem_regions(
            self._uch, ctypes.byref(regions), ctypes.byref(count)
        )
        if status != uc.UC_ERR_OK:
            raise UcError(status)
...
class UcContext:
    ...
    def reg_read(self, reg_id, opt=None):
        return reg_read(
            functools.partial(_uc.uc_context_reg_read, self._context),
            self.arch,
            reg_id,
            opt,
        )
    ...
    def reg_write(self, reg_id, value):
        return reg_write(
            functools.partial(_uc.uc_context_reg_write, self._context),
            self.arch,
            reg_id,
            value,
        )
    ...
    def __setstate__(self, state):
        self._size = state[1]
        self._context = ctypes.cast(
            ctypes.create_string_buffer(state[0], self._size), uc_context
        )
...
UC_HOOK_INSN_SYS_TYPE = Callable[
    [Uc, int, Tuple[int, int, int, int, int, int], Any], int
]
